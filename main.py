...
def create_pdf(data: dict) -> str:
    ...
    story.append(Paragraph(f"<b>Дата составления:</b> {today}", normal_style))
    story.append(Spacer(1, 8))

    story.append(Paragraph("<b>Истец / кредитор:</b>", normal_style))
    story.append(Paragraph(
        f"{data['claimant_name']}<br/>"
        f"ИНН: {data['claimant_inn']}<br/>"
        f"КПП: {data['claimant_kpp']}<br/>"
        f"ОГРН: {data['claimant_ogrn']}<br/>"
        f"Адрес: {data['claimant_address']}",
        normal_style,
    ))
    ...
    story.append(Paragraph(
        f"{data['defendant_name']}<br/>"
        f"ИНН: {data['defendant_inn']}<br/>"
        f"КПП: {data['defendant_kpp']}<br/>"
        f"ОГРН: {data['defendant_ogrn']}<br/>"
        f"Адрес: {data['defendant_address']}",
        normal_style,
    ))
    ...
    story.append(Paragraph(
        ...
        f"на основании следующего документа: <b>{data['contract']}</b>.",
        normal_style,
    ))

    story.append(Paragraph(
        f"Основание возникновения задолженности: <b>{data['reason']}</b>.",
        normal_style,
    ))
    ...
    story.append(Paragraph(
        ...
        f"<b>{money_format(data['amount'])} руб.</b>",
        normal_style,
    ))
...
async def finish_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        print("PDF ERROR:", e)
        await update.message.reply_text("Не удалось сформировать PDF. Попробуйте ещё раз.")
